...
import kotlinx.android.synthetic.main.ware_material_position.*
import okhttp3.*
...
import ykk.xc.com.wms.warehouse.adapter.Ware_Material_Position_Adapter
 ...
 * 拣货位置
 */
class Ware_Material_PositionActivity : BaseActivity() {
    ...
    private var mAdapter: Ware_Material_Position_Adapter? = null
    ...
    private class MyHandler(activity: Ware_Material_PositionActivity) : Handler() {
        private val mActivity: WeakReference<Ware_Material_PositionActivity>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    SUCC1 -> { // 查询分录 进入
                        m.listDatas.clear()
                        val list = JsonUtil.strToList(msgObj, InventoryNow::class.java)
    ...
    override fun setLayoutResID(): Int {
        return R.layout.ware_material_position
    }
    ...
    override fun initView() {
        ...
        mAdapter = Ware_Material_Position_Adapter(context, listDatas)
    ...
    private fun run_smDatas(strBarcode :String) {
        ...
        var mUrl = getURL("inventoryNow/findListByParam")
        val formBody = FormBody.Builder()
                .add("barcode", getValues(et_code).trim())
                .add("strBarcode", strBarcode)
                .add("columnName","E.stockPositionNumber") // 按照库位排序
                .add("sortWay","ASC")
                .build()
