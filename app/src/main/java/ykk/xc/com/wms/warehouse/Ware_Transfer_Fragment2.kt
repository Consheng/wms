...
//                                    Comm.showWarnDialog(m.mContext,"请先保存当前数据！")
                    ...
                    SUCC2 -> { // 查询库存 进入
                        ...
                        m.tv_stockQty.text = Html.fromHtml("即时库存：<font color='#6a5acd'>"+m.df.format(list[0].avbQty)+"</font>")
                    }
                    UNSUCC2 -> { // 查询库存  失败
...
//        tv_num.text = if(icEntry.fqty > 0) df.format(icEntry.fqty) else ""
        tv_unitName.text = Html.fromHtml("单位：<font color='#6a5acd'>"+icEntry.unitName+"</font>")
    ...
    private fun run_smDatas(mtlId : Int) {
        ...
        when(smqFlag) {
            ...
            '4' -> { // 选择物料后，查询的数据
                ...
                strStockGroup = icStockBillEntry.fscStockId.toString() +","+ icStockBillEntry.stockAreaId2_wms +","+
                        icStockBillEntry.storageRackId2_wms +","+ icStockBillEntry.stockPosId2_wms
            }
        }
