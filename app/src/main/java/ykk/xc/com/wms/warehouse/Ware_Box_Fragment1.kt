...
import kotlinx.android.synthetic.main.ware_box_fragment1.*
import okhttp3.*
...
import ykk.xc.com.wms.warehouse.adapter.Ware_Box_Fragment1_Adapter
 ...
 * 作者：ykk
 */
class Ware_Box_Fragment1 : BaseFragment() {
    ...
    private var mAdapter: Ware_Box_Fragment1_Adapter? = null
    private var mContext: Activity? = null
    private var user: User? = null
    private var boxBarCode:BoxBarCode? = null
    private var boxBarCode2:BoxBarCode? = null
    private val df = DecimalFormat("#.######")
    private var parent: Ware_Box_MainActivity? = null
...
//    private var bt :BarCodeTable? = null  // 记录当前物料扫描返回的对象
    private var icItem :ICItem? = null  // 记录当前物料扫描返回的对象


    // 消息处理
    private val mHandler = MyHandler(this)
    private class MyHandler(activity: Ware_Box_Fragment1) : Handler() {
        private val mActivity: WeakReference<Ware_Box_Fragment1>
...
//                                }
                            ...
                            '3'-> { // 物料
                                ...
                                if(icItem.batchManager.equals("Y")) {
                                    val bundle = Bundle()
                                    bundle.putString("mtlName", icItem.fname)
...
//                            m.reset(1)
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_box_fragment1, container, false)
    }

    override fun initView() {
        mContext = getActivity()
        parent = mContext as Ware_Box_MainActivity

        recyclerView.addItemDecoration(DividerItemDecoration(mContext, DividerItemDecoration.VERTICAL))
        recyclerView.layoutManager = LinearLayoutManager(mContext)
        mAdapter = Ware_Box_Fragment1_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : Ware_Box_Fragment1_Adapter.MyCallBack {
...
//              if(it.icItem.batchManager.equals("Y") || it.icItem.snManager.equals("Y")) {
            ...
            // 判断相同的物料
            for ((index,it) in checkDatas.withIndex()) {
                if (icItem!!.fitemid == it.fitemId) {
                    isBool = true
                    if (it.icItem.batchManager.equals("Y")) { // 启用批次号
                        setBatchCode(index, icItem!!.batchCode, icItem!!.realQty)

                    } else if (it.icItem.snManager.equals("Y")) { // 启用序列号
                        setSnCode(index, icItem!!.snCode)
            ...
            if(!isBool) { // 添加新行
                var fqty = if (icItem!!.realQty > 0) icItem!!.realQty else 1.0
                addRow(icItem!!.batchCode, icItem!!.snCode, fqty)
            }
        } else { // 添加新行
            var fqty = if (icItem!!.realQty > 0) icItem!!.realQty else 1.0
            addRow(icItem!!.batchCode, icItem!!.snCode, fqty)
     ...
     * 新增行
     */
    private fun addRow(batchCode:String, snCode:String, barcodeQty:Double) {
        val mbr = MaterialBinningRecord()
        mbr.type = '3' // 1:生产装箱，2:销售装箱，3:库内装箱
        mbr.packType = 'A'
        ...
        mbr.fitemId = icItem!!.fitemid
        mbr.fsourceInterId = 0
        mbr.fsourceEntryId = 0
        mbr.fsourceNo = ""
        mbr.fsourceQty = 0.0
        mbr.fsourceHightLimitQty = 0.0
        ...
        if (icItem!!.batchManager.equals("Y")) { // 启用批次号
            setBatchCode(pos, batchCode, barcodeQty)

        } else if (icItem!!.snManager.equals("Y")) { // 启用序列号
