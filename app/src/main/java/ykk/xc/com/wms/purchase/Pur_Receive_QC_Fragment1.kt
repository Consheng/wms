...
import kotlinx.android.synthetic.main.pur_receive_qc_fragment1.*
import kotlinx.android.synthetic.main.pur_receive_qc_main.*
import okhttp3.*
 ...
 * 作者：ykk
 */
class Pur_Receive_QC_Fragment1 : BaseFragment() {
    ...
    companion object {
        ...
        private val SEL_QUALIFIED_STOCK = 17
        private val SEL_UN_QUALIFIED_STOCK = 18
    ...
    private var parent: Pur_Receive_QC_MainActivity? = null
...
//    var isReset = false // 是否点击了重置按钮.
    ...
    private class MyHandler(activity: Pur_Receive_QC_Fragment1) : Handler() {
        private val mActivity: WeakReference<Pur_Receive_QC_Fragment1>
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    FIND_SOURCE ->{ // 查询源单 返回
                        ...
                        m.icStockBill.supplier = list[0].poInStock.supplier
...
//        tv_qualifiedStockSel.text = m.qualifiedStock.stockName
        tv_unQualifiedStockSel.text = m.unQualifiedStock.stockName
        // 重量单位类型(1：千克，2：克，3：磅)
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.pur_receive_qc_fragment1, container, false)
    }

    override fun initView() {
        mContext = getActivity()
        parent = mContext as Pur_Receive_QC_MainActivity
    }
    ...
    override fun initData() {
        ...
        icStockBill.billType = "CGSHCZ" // 仓库收货操作
    ...
    @OnClick(R.id.tv_inDateSel, R.id.tv_suppSel, R.id.tv_deptSel, R.id.tv_emp1Sel, R.id.tv_emp2Sel, R.id.tv_emp3Sel, R.id.tv_emp4Sel,
             R.id.btn_save, R.id.btn_clone, R.id.tv_weightUnitType, R.id.tv_connBlueTooth, R.id.tv_roughWeight, R.id.tv_qualifiedStockSel, R.id.tv_unQualifiedStockSel)
    fun onViewClicked(view: View) {
        ...
        when (view.id) {
            ...
            R.id.tv_qualifiedStockSel -> { // 选择 合格仓库
//                showForResult(Stock_DialogActivity::class.java, SEL_QUALIFIED_STOCK, null)
            }
            R.id.tv_unQualifiedStockSel -> { // 选择 不合格仓库
                showForResult(Stock_DialogActivity::class.java, SEL_UN_QUALIFIED_STOCK, null)
            }
...
//        if (icstockBill.qualifiedStockId == 0) {
//            Comm.showWarnDialog(mContext, "请选择合格仓库！")
//            return false
//        }
        if (icStockBill.unQualifiedStockId == 0) {
            Comm.showWarnDialog(mContext, "请选择不合格仓库！")
            return false
        }
//        if(icstockBill.qualifiedStockId == icstockBill.unQualifiedStockId) {
//            Comm.showWarnDialog(mContext, "合格、不合格仓库不能相同！")
//            return false
//        }
...
//            SEL_QUALIFIED_STOCK -> { // 合格仓库     返回
//                val qualifiedStock = data!!.getSerializableExtra("obj") as Stock
//                tv_qualifiedStockSel.text = qualifiedStock!!.fname
//                icstockBill.qualifiedStockId = qualifiedStock.fitemId
//            }
            SEL_UN_QUALIFIED_STOCK -> { // 不合格仓库     返回
                val unQualifiedStock = data!!.getSerializableExtra("obj") as Stock
                tv_unQualifiedStockSel.text = unQualifiedStock!!.fname
                icStockBill.unQualifiedStockId = unQualifiedStock.fitemId
            }
            SEL_EMP1 -> {//查询业务员	返回
                if (resultCode == Activity.RESULT_OK) {
